...
class ClickHouse(Base):
    ...
    async def check(self):
        if not await aioshutil.which("clickhouse-backup"):
            raise RuntimeError("clickhouse-backup not found in PATH")
        return True

    @classmethod
    def _check_error(cls, std: bytes):
        if std and "[ERROR]" in std.decode():
            raise RuntimeError(f"clickhouse-backup failed: {std.decode()}")
